import { Component, OnInit, OnDestroy, ViewChild, ElementRef, Inject, Optional } from '@angular/core';
...
export class TemplateWizardComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    @Optional() private dialogRef: MatDialogRef<TemplateWizardComponent>,
    @Optional() @Inject(MAT_DIALOG_DATA) public data: WizardDialogData | null
  ) {
    this.templateForm = this.createForm();
  }

  ngOnInit(): void {
    // Determinar el modo basado en la ruta o datos del diálogo
    if (this.data) {
      // Modo diálogo
  ...
  private loadTemplateById(templateId: number): void {
  ...
  async saveTemplate(): Promise<void> {
    ...
    try {
      ...
      if (this.dialogRef) {
        // Modo diálogo - cerrar con resultado
        this.dialogRef.close(result);
      } else {
        // Modo ruta - navegar de vuelta a la lista
        this.router.navigate(['/documentos/plantillas']);
      }
    } catch (error) {
